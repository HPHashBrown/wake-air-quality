...
# ============================================
st.set_page_config(
    page_title="Project AIR - Atmospheric Intelligence & Response",
    page_icon="🌐",
    layout="wide",
...
if "GEM_KEY" in st.secrets and st.secrets["GEM_KEY"]:
    try:
        ...
        st.sidebar.warning(f"Error initializing Client: {e}")
        client = None
else:
...
def fetch_30day_pm25_history(lat, lon):
    ...
    try:
        response = requests.get(aq_url, timeout=15).json()
...
with tab1:
    ...
    # 30-Day Historical Trend Display
    st.markdown("---")
    st.subheader("📅 30-Day Ambient PM2.5 Historical Trend")
    curr_lat, curr_lon = st.session_state.map_center
    with st.spinner("Fetching 30-day historical data..."):
        df_30day = fetch_30day_pm25_history(curr_lat, curr_lon)
    
    if not df_30day.empty:
        df_30day['Date'] = df_30day['time'].dt.date
        df_daily_avg = df_30day.groupby('Date')['pm25'].mean().reset_index()
        ...
        st.info("30-day historical ambient trend dataset currently unavailable for this coordinate zone.")
...
with tab8:
    ...
    # Safe evaluation of commute metrics directly linked to Session State
    if 'route_data' in st.session_state and st.session_state.route_data is not None:
        rdata = st.session_state.route_data
        col1, col2 = st.columns(2)
        aqi_val_route = round(rdata.get('exposure', 0.0), 1)
